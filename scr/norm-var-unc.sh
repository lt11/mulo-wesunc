...
### the one that normalises and de-duplicates the unmatched variants
...
g_out_dir="${base_dir}/var-calls/norm-gatk-unc"
if [[ -d "${g_out_dir}" ]]; then rm -rf "${g_out_dir}"; fi
mkdir -p "${g_out_dir}"
v_out_dir="${base_dir}/var-calls/norm-vardict-unc"
if [[ -d "${v_out_dir}" ]]; then rm -rf "${v_out_dir}"; fi
mkdir -p "${v_out_dir}"

### input folders
g_dir="${base_dir}/var-calls/gatk-unc"
v_dir="${base_dir}/var-calls/vardict-unc"
...
echo "Running the one that normalises and de-duplicates \
